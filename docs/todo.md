...
- `P1` 提升餐厅推荐匹配度。
  - 当前餐食偏好会影响候选类型纳入和 AI/本地排序；本地评分已按菜系和人均预算加权，并已排除奶茶/茶饮、咖啡馆、面包烘焙和甜品店。
  - 需要进一步结合用餐时间、评分、营业时段，以及午餐/晚餐时段是否可用。
  - 可参考大众点评式菜系分类，但要保持数据来源合法和技术上可控。

- `P1` 扩展开放时间解析。
  - 当前红色提示已支持多个时间段、跨夜、中文星期开放/闭馆文本和“全天开放”/“24小时”。
  - 还需要进一步识别预约制说明、临时公告、节假日例外、特殊闭馆日和 provider 的模糊格式。
