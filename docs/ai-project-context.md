...
- `npm run smoke:deepseek-limit`：DeepSeek 用量限制生产 smoke。默认 dry-run；只有追加 `-- --execute` 才会创建临时生产用户并写入 `route_ai_runs` 触发用户日限额，结束后默认清理临时数据。该命令需要 `SUPABASE_SERVICE_ROLE_KEY`，必须在用户明确授权后执行。
...
- DeepSeek 启用时，体验站点可生成更长的建筑、历史、城市记忆、核验建议和打卡任务；失败或未启用时使用未核验模板 fallback。路线详情页和途中模式会显示事实核验提示，AI 返回的 `sourceClaims` 只作为待核验线索，不作为官方来源；站点事实核验 profile 会补充官方公告搜索、门票/预约状态、闭馆提醒、来源链接和最近核验时间。
...
- 阶段 4/5：在用户明确授权后执行 `npm run smoke:deepseek-limit -- --execute`，验证 AI 用量限制生产登录 allowed 和用户日限额拒绝。
