...
- `src/components/routes/fact-check-note.tsx`：深读内容事实核验提示；AI claim 只作为待核验线索展示，并结合站点资料展示官方公告、门票/预约、闭馆提醒、来源引用和最近核验时间。
...
- Edge Function：`deepseek-proxy`，用于 DeepSeek JSON-mode intent 解析、候选排序、路线标题、站点深读、AI 用量限制和 provider 诊断。站点深读会先用 `BAIDU_AI_SEARCH_API_KEY` 调用百度 AI 搜索，再筛选政府/场馆、档案/学术、权威媒体来源，并将来源摘要、链接和检索时间传给 DeepSeek；无法获得这类来源时只允许高德地点资料形成“部分核验”，否则回退为不含事实断言的本地模板。客户端 schema 校验失败时会把错误摘要和原始 JSON 发回该函数自动修复一次；函数已部署并通过 Function secrets 配置 `AI_DAILY_USER_LIMIT=30`、`AI_PROJECT_COST_LIMIT_CNY=20`。`DEEPSEEK_API_KEY` 和 `BAIDU_AI_SEARCH_API_KEY` 只能放在 Supabase Function secrets 中。
...
- `/plan/` Complete 模式可生成 10-15 个沿途候选，按绕行和主题适配评分，并支持加入、备用、忽略。候选生成会从路线站点和 provider polyline 中按距离抽样，长路段补充中间搜索中心，密集城区按距离去重并限制查询上限，再通过高德附近 POI 搜索映射成路线候选。单个采样点失败不再导致整个候选生成 fallback。
...
- DeepSeek 启用时，体验站点先检索政府/场馆、档案/学术和权威媒体来源，并把来源摘要、链接和检索时间作为唯一事实证据交给 DeepSeek；文案面向首次到访的步行游客，以现场可观察的画面和经证据支持的城市故事串联，不使用营销软文、堆砌形容词或无来源绝对结论。每个体验站点提供恰好 2 项可选闯关式打卡任务；餐饮的营业、价格、菜单和预约信息仅在来源明确且带检索日期时出现。路线详情页和途中模式会显示核验状态、检索时间、已用来源和出发日核验入口；只有高德资料时显示“部分核验”，无可靠资料时回退为不含事实断言的本地模板。
