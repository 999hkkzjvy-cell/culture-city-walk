...
- 路线阅读页显示路段来源，可渲染高德 JS 地图、站点 marker、provider/estimated polyline，并可通过 `amap-proxy` 复核步行、公交、驾车/打车路段。地图只读取路线中的坐标；时间线 hover/focus 会高亮选中站点和到达该站的路段。确认站点和候选 POI 会在路线/候选保存时单独持久化到 `places`。
...
- 阶段 4/5：部署并验证 AI 用量限制、加强高德元数据之外的事实核验、改进地图视野和候选插入预览。
