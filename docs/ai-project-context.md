...
- `src/components/routes/fact-check-note.tsx`：深读内容事实核验提示；AI claim 只作为待核验线索展示。
...
- Edge Function：`deepseek-proxy`，用于 DeepSeek JSON-mode intent 解析、候选排序、路线标题和站点深读。客户端 schema 校验失败时会把错误摘要和原始 JSON 发回该函数自动修复一次；函数代码已支持 `AI_DAILY_USER_LIMIT` 和 `AI_PROJECT_COST_LIMIT_CNY`，但新版函数仍需部署后线上验证。`DEEPSEEK_API_KEY` 只能放在 Supabase Function secrets 中。
...
- 路线阅读页显示路段来源，可渲染高德 JS 地图、站点 marker、provider/estimated polyline，并可通过 `amap-proxy` 复核步行、公交、驾车/打车路段。地图只读取路线中的坐标；时间线 hover/focus 会高亮选中站点和到达该站的路段。规划页右侧路线摘要复用紧凑版地图，候选卡片 hover/focus/展开时会显示候选点和插入前后虚线路段。确认站点和候选 POI 会在路线/候选保存时单独持久化到 `places`。
...
- DeepSeek 启用时，体验站点可生成更长的建筑、历史、城市记忆、核验建议和打卡任务；失败或未启用时使用未核验模板 fallback。路线详情页和途中模式会显示事实核验提示，AI 返回的 `sourceClaims` 只作为待核验线索，不作为官方来源。
...
- AI 协作在 `NEXT_PUBLIC_DEEPSEEK_PROXY_ENABLED=true` 时调用 `deepseek-proxy`；否则使用确定性本地 fallback。DeepSeek 返回内容未通过前端 schema 校验时，会自动带错误摘要重试修复一次；第二次仍失败才进入 fallback。登录用户的 intent 解析、候选排序和路线标题会记录到 `route_ai_runs`，包括 prompt 版本、model、token、耗时、估算成本和幂等键。函数代码已实现用户每日次数和项目日成本限制，待部署新版 `deepseek-proxy` 和线上烟测后才视为启用。
...
- 阶段 4/5：部署并验证 AI 用量限制、加强高德元数据之外的事实核验、继续打磨地图编辑过程中的视野保持策略。
