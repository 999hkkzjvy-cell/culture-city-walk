...
Cultural Citywalk 是一个以“可信知识、统一主题、现场可完成”为核心的城市文化伴游。产品先聚焦南京三条人工审核主题路线；AI 用于资料驱动的深读、现场问答和受约束的路线微调，不应把附近但缺乏主题和资料支撑的 POI 拼成路线。
...
2026-07-23 已部署深读 v0.6 与站点问答：线上匿名 `diagnostic` 返回 `edgeFunctionReachable=true`、`deepseekKeyConfigured=true`、`sourceResearchConfigured=true`、`dailyUserLimit=30`、`projectCostLimit=20`。本次仅验证配置与函数可达性，未额外发起会产生模型费用的生产深读请求。
...
- 2026-07-23 起进入“城市伴游优先”阶段：主导航为探索路线、规划路线、我的路线；南京探索页只展示三条待人工审核的主题编辑稿。详细产品边界、审核清单与 Beta 标准见 `docs/next-phase-product-direction.md`。
- 深读 prompt 为 `route-collaboration-v0.6`。资料充分时输出摘要加四段导览（800–1200 字）；资料有限时缩短，不编造内容。深读和现场问答可展示百度检索的调用统计；已生成深读写入 localStorage，供弱网阅读。现场问答需要登录，问答记录不上传为永久聊天档案。
...
- `/recommendations/` 是两栏式推荐路线浏览页，左侧支持城市文本、主题、节奏和时长筛选；推荐路线覆盖南京、上海、北京、广州、杭州、苏州、成都、西安，并展示路线预览站点、适合转发的短文案和适用场景标签。
...
- AI 协作在 `NEXT_PUBLIC_DEEPSEEK_PROXY_ENABLED=true` 时调用 `deepseek-proxy`；否则使用确定性本地 fallback。DeepSeek 返回内容未通过前端 schema 校验时，会自动带错误摘要重试修复一次；第二次仍失败才进入 fallback。登录用户的 intent 解析、候选排序和路线标题会记录到 `route_ai_runs`，包括 prompt 版本、model、token、耗时、估算成本和幂等键。候选排序支持含餐模式下的 15 个候选，并会把一句话路线目标传给 `deepseek-proxy`；本地 fallback 也会识别午餐/晚餐并优先排序餐厅。函数已部署用户每日次数和项目日成本限制，未登录诊断、生产登录 allowed 和用户日限额拒绝 smoke 均已通过。
