...
- Edge Function：`deepseek-proxy`，用于 DeepSeek JSON-mode intent 解析、候选排序、路线标题、站点深读、AI 用量限制和 provider 诊断。客户端 schema 校验失败时会把错误摘要和原始 JSON 发回该函数自动修复一次；函数已部署并通过 Function secrets 配置 `AI_DAILY_USER_LIMIT=30`、`AI_PROJECT_COST_LIMIT_CNY=20`。`DEEPSEEK_API_KEY` 只能放在 Supabase Function secrets 中。
...
- DeepSeek 启用时，体验站点可生成更长的类型化深读：名人 / 文学站点补充人物生平、作品、轶事和相关人物，建筑站点补充风格与细节，博物馆站点补充馆藏和展陈线索，餐馆站点补充沿革与菜式；每个体验站点提供恰好 2 项可选闯关式打卡任务。失败或未启用时使用同样按站点类型生成的未核验模板 fallback。路线详情页和途中模式会显示事实核验提示，AI 返回的 `sourceClaims` 只作为待核验线索，不作为官方来源；站点事实核验 profile 会补充官方公告搜索、门票/预约、闭馆提醒、来源引用和最近核验时间。
