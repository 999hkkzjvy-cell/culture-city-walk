...
- 规划页必须从 provider 结果选择出发/必去/终点；自由文本手工添加按钮已移除，按 Enter 会触发高德搜索。规划流程包含起始时间和可选含餐问题。含餐时自动纳入餐厅候选，并把人数、人均、菜系和一句话路线目标交给 AI/本地排序；本地评分会按菜系、人均预算、预计用餐时段、营业状态和评分加权，餐厅候选明确排除奶茶/茶饮/冷饮、咖啡馆、面包烘焙和甜品店。路线标题先用本地规则生成，DeepSeek 可用时会在生成沿途候选后异步生成 32 字以内的短标题。
...
- AI 协作在 `NEXT_PUBLIC_DEEPSEEK_PROXY_ENABLED=true` 时调用 `deepseek-proxy`；否则使用确定性本地 fallback。DeepSeek 返回内容未通过前端 schema 校验时，会自动带错误摘要重试修复一次；第二次仍失败才进入 fallback。登录用户的 intent 解析、候选排序和路线标题会记录到 `route_ai_runs`，包括 prompt 版本、model、token、耗时、估算成本和幂等键。函数已部署用户每日次数和项目日成本限制，未登录诊断已通过；正常调用与超额拒绝的生产 smoke 需要用户明确授权后再用临时用户验证。
