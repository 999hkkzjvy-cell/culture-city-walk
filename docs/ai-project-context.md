...
- `src/components/routes/shared-route-reader.tsx`：只读分享路线加载器。
...
- `src/lib/storage.ts`：localStorage 草稿、路线预览、候选状态、途中进度、本地打卡图和本地收藏路线。
...
- `/journey/` 是两栏途中模式：左侧路线概览，右侧选中站点深读、打卡任务、实践提示、高德导航、到达/跳过和打卡图上传。图片先压缩后存入 localStorage；云端 UUID 路线且用户登录时，会同步到私有 `route-media` 和 `route_checkin_photos`。
- 途中模式只按体验站点统计进度。到显式终点后，可完成路线并按到达站点和上传打卡图生成分数与夸夸文案。
- 路线详情页收藏按钮有可见的本地 `已收藏` 状态。分享路线展示深读文案但不展示打卡任务，并可本地收藏。路线库 `我的收藏` 读取本地收藏，并提供 `查看` 和 `修改`，修改会把路线载入 `/plan/`。
